	...
	public Model(ConfigMashup config) {
		...
		if (createdAt != null) {
			...
			try {
				...
				LoggerMP.writeError("[Model] couldn't parse config param '"
						+ Model.PARAM_CREATED_AT + "'" + " for mashup "
						+ page.getMashupIdent());
			}
		}

		numberRecordsPerPage = parseInt(config
				.getValue(PARAM_NUMBER_RECORDS_PER_PAGE));
	}
