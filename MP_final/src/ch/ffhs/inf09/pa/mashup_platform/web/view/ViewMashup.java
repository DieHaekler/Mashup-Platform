...
/**
 * The ViewMashup class produces the HTML code of a single mashup page.
 * 
 * @author Joël
 * 
 */
public class ViewMashup extends ViewApplication {
	public ViewMashup(ModelMashup model) throws ExceptionMP {
		...
		if (page != null && info != null) {
			...
			content = content.replace("[__MASHUP_PAGINATION_MAX__]", ""
					+ numberPages);
			content = content.replace("[__MASHUP_PAGINATION_CURRENT__]", ""
					+ pagenr);
			...
			// insert html code
			part = getTemplate("html/mashup/" + page.getMashupIdent()
					+ "/content.html");
